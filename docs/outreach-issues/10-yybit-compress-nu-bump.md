Heads-up from the [Numan](https://github.com/numan-cli/numan) catalog triage.
...
**Ask:** if/when you bump to a modern `nu-plugin` / `nu-protocol` (0.114.x preferred), please cut a tagged release. Uploaded target archives are a bonus; tag + Cargo pins are enough for [numan-plugins](https://github.com/numan-cli/numan-plugins) to CI-build and hand assets to the registry.
