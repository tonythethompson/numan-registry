...
nupm already consumes this repo via **git revision pins**, which fits a monorepo. Numan wants a downloadable artifact per entry, so I built mirrors on my side rather than using `/archive/{commit}.zip` (same byte-stability concerns as tag archives).
...
Not looking for a drive-by Actions PR. This repo's release story is yours. Mostly want to know if I'm fighting the grain by mirroring, or if there's a lightweight convention you'd prefer (even if it's "please keep mirroring").
